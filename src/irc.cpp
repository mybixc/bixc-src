...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BixCTEST2\r");
            Send(hSocket, "WHO #BixCTEST2\r");
        } else {
            // randomly join #BixC00-#BixC05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BixC%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BixC%02d\r", channel_number).c_str());
        }
