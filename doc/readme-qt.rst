...
Alternatively, install Qt Creator and open the `BixC-qt.pro` file.

An executable named `BixC-qt` will be built.
...
A warning for people using the *static binary* version of BixC on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BixC is linked against libdb5.3.
...
installed causes BixC-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
